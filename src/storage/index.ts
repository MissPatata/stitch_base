...
export async function saveDesigns(designs: Design[]): Promise<void> {
  await storageAPI.writeFile(
    `${BASE_DATA_DIR}/designs.json`,
...
export async function loadPatterns(): Promise<Pattern[]> {
  try {
    ...
    // Ensure all patterns have the images property (migration for existing data)
...
export async function removeDesignImage(image: DesignImage): Promise<void> {
  const fullPath = window.electronAPI
    ? `${BASE_DATA_DIR}/${image.filePath}`
    : image.filePath;
...
export async function savePatternImage(
  ...
  try {
    ...
    console.error("Error saving pattern image:", error);
    throw error;
  }
}

export async function removePatternImage(image: PatternImage): Promise<void> {
  const fullPath = window.electronAPI
    ? `${BASE_DATA_DIR}/${image.filePath}`
    : image.filePath;
  await storageAPI.deleteFile(fullPath);
...
export async function savePatternFile(
  patternId: string,
  file: File
): Promise<PatternFile> {
  const fileId = nanoid();
...
export async function removePatternFile(
  patternFile: PatternFile
): Promise<void> {
  const fullPath = window.electronAPI
    ? `${BASE_DATA_DIR}/${patternFile.filePath}`
